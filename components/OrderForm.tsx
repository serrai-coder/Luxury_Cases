...
const OrderForm: React.FC<OrderFormProps> = ({ onOrderSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // !!! تنبيه أمني هام !!!
    // لا تستخدم هذا الكود في موقع منشور على الإنترنت.
    // مفتاح API الخاص بك سيكون مكشوفاً للجميع. هذا الحل مخصص للتطوير والاختبار فقط.
    ...
    if (NOTION_API_KEY.startsWith('أدخل_') || DATABASE_ID.startsWith('أدخل_')) {
      setErrors({ submit: 'الرجاء إعداد مفتاح Notion API ومعرف قاعدة البيانات في الكود أولاً.' });
      setIsLoading(false);
      return;
    }
    ...
    // 3. إعداد بيانات الطلب لإرسالها إلى Notion
    ...
    // 4. إرسال الطلب عبر وسيط لتجاوز قيود المتصفح الأمنية
